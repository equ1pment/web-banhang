// --- IMPORTS ---
import {
    getProducts, getProductImports, getProfitSettings, 
    productTypes, getCart, saveCart
...
// --- UI: FILTER POPULATION ---
function populateFilters(initialSearchTerm, initialCategoryId) {
    ...
    if (initialCategoryId) {
        ...
        categorySelect.prepend(new Option("Tất cả danh mục", ""));
        categorySelect.value = "";
    }
}

// --- CORE LOGIC: FILTER, SORT, AND RENDER PRODUCTS ---
function performSearchAndRender() {
    ...
    const category = productTypes.find(t => t.id === categoryId);
    const titleName = category ? category.name : 'Tất cả sản phẩm';
    const titleHtml = `<h2 class="section-title">Danh mục: ${titleName} (Tìm thấy ${results.length} sản phẩm)</h2>`;

    let productsHtml = '';
    if (results.length === 0) {
        productsHtml = "<p style='color: white; text-align: center; width: 100%;'>Không có sản phẩm nào hợp lệ.</p>";
    } else {
...
// --- EVENT HANDLING: ADD TO CART ---
function handleCartEvents() {
    const productContainer = document.getElementById('products-list-container');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    populateFilters(searchTerm, categoryId);
